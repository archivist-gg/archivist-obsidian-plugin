...
const KINDS_28 = [
  "initiative-bonus", "skill-bonus", "half-proficiency", "immune-condition", "resistance", "hp-per-level-bonus", "speed-bonus", "attunement-limit", "sense",
...
describe("featureEffectSchema · the 28-arm union (R4-G1a D1, G1; R4-G6b §5.2)", () => {
  it("declares exactly the 28 kinds, in order", () => {
    const kinds = featureEffectSchema.options.map((o) => (o.shape.kind as { value: string }).value);
    expect(kinds).toEqual([...KINDS_28]);
  });
...
const _kindsAgree: Equals<FeatureEffect["kind"], (typeof KINDS_28)[number]> = true;
void _kindsAgree;
