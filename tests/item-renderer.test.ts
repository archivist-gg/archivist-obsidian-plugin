...
describe("renderItemBlock — formatAttunement structured form (Bug A)", () => {
  it("formats canonical structured attunement { required: true } as Required", () => {
    const root = mountContainer();
    const card = renderItemBlock({
  ...
  it("formats { required: false } as Not Required", () => {
    const root = mountContainer();
    const card = renderItemBlock({
      name: "Bag of Holding",
      type: "Wondrous item",
  ...
  it("formats { required: true, restriction: 'by a wizard' } showing the restriction", () => {
    const root = mountContainer();
    const card = renderItemBlock({
...
describe("renderItemBlock — mechanical summary hidden from card (Bug B)", () => {
  it("does NOT render the mechanical summary section in the item card", () => {
    const root = mountContainer();
    const card = renderItemBlock({
      name: "Bracers of Defense",
      type: "Wondrous item",
