...
const spell = { name: "Fire Bolt" } as never;
...
function setup() {
  ...
  const plugin = {
    ...
    compendiumManager: { getWritable: () => [{ name: "Me" }], saveEntity: vi.fn().mockResolvedValue({ slug: "me_spell_x" }) },
  };
...
describe("renderSpellEditMode on a readonly host (R4-G6b §3.4)", () => {
  ...
  it("hostReadonly: Save-as-new saves the new entity and NEVER writes the host note", async () => {
    ...
    renderSpellEditMode(spell, el, ctx as never, plugin as never, onExit, undefined, undefined, true);
  ...
  it("without the flag Save-as-new replaces the fence with the ref (the control)", async () => {
    const { el, editor, plugin, ctx, onExit } = setup();
    renderSpellEditMode(spell, el, ctx as never, plugin as never, onExit, { slug: "old_spell_x", compendium: "Me", readonly: false }, undefined);
    (el.querySelector(".archivist-side-btn-save-as-new") as HTMLElement).click();
    await Promise.resolve(); await Promise.resolve();
    expect(editor.replaceRange).toHaveBeenCalledTimes(1);
    expect(editor.replaceRange.mock.calls[0][0]).toContain("{{spell:me_spell_x}}");
    expect(plugin.compendiumManager.saveEntity).toHaveBeenCalledTimes(1);
  });
});
