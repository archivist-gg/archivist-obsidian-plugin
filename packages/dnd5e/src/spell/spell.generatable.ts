...
export const spellGeneratable: Generatable = {
  type: "spell",
  description:
  ...
  instructions: `In spell description text, use inline formula tags for damage and save DCs where applicable:
...
Example description entry: "Each creature in a 20-foot radius must make a \`dc:WIS\` Dexterity saving throw. A target takes \`damage:8d6\` fire damage on a failed save, or half as much on a success."`,
  inputSchema: spellInputSchema,
  enrich: (input) => enrichSpell(input as Record<string, unknown>),
};
