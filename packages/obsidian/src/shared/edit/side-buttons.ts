...
/**
 * The two readonly facts are ORTHOGONAL (R4-G6b §3.2):
 * `isReadonly` = the compendium ENTITY a `{{type:slug}}` ref points at is readonly (it gates the compendium Save in
 * the `compendium-pending` state); `isHostReadonly` = the NOTE this block lives in belongs to a readonly compendium
 * (it hides Delete and the in-place Save, leaving Save-as-new the only write).
 */
...
export function renderSideButtons(container: HTMLElement, config: SideButtonConfig): void {
  ...
  if (config.state === "pending") {
    // R4-G6b §3.2: on a readonly compendium note the ONLY write is a new note elsewhere (invariant 10), so the bar is
    // exactly Save-as-new (the `compendium-pending` branch's markup) and Cancel (this branch's own markup).
    if (config.isHostReadonly) {
      const saveAsNewBtn = container.createDiv({ cls: "archivist-side-btn archivist-side-btn-save-as-new" });
      setIcon(saveAsNewBtn, "save");
      saveAsNewBtn.createSpan({ cls: "archivist-plus-overlay", text: "+" });
      saveAsNewBtn.title = "Save as new entity";
    ...
    // R4-G6b §3.2: a readonly compendium note's block never offers Delete (invariant 10).
    if (!config.isHostReadonly) {
