...
| `--scroll-capture [sel]` | Screenshots each viewport-full of the scroll container (up to 20 steps, stopping at the bottom), then restores `scrollTop`. NOT `.pc-content`, which carries no `overflow-y`. **With no selector, resolution is LEAF-SCOPED**: it searches only inside the `.workspace-leaf` holding `--selector` (falling back to `.workspace-leaf.mod-active`), trying `.markdown-preview-view`, `.cm-scroller`, `.view-content`, and the first that actually scrolls wins. With a selector, resolution is the plain document-global `querySelector`, first match. FAILS the run on: a container matching nothing, a container that cannot scroll, fewer than TWO shots, or a `scrollTop` left unrestored. Runs ONCE, at natural width, after any `--widths` sweep has restored the pane. | `--scroll-capture` |
...
- **Hidden ROOT.** Per the HTML specification, `innerText` on an element that is **not being rendered** returns its `textContent`. Measured live: an inactive `#panel-passive` returned `innerText` and `textContent` both **12002 characters**, identical, not the empty string. Every hidden panel behaved the same way.
- **Hidden DESCENDANT of a rendered root.** These really are skipped. Measured on the same sheet, the ACTIVE `#panel-actions` returned **621** characters of `innerText` against **7633** of `textContent`. That gap is also what proves the reading was taken with layout live rather than flattened.
