...
3. **At least one functional assertion** that the feature is actually present: `--assert-selector <sel>` and/or `--assert-text <text>`. **Scope the selector to a panel** whenever the assertion is about a specific tab (`--assert-selector "#panel-passive .foo"`). `--assert-selector` counts DOCUMENT-WIDE, so a bare selector is satisfied by a match in any panel including a hidden one, and this item can therefore be marked done on an element the tab under test never rendered. See the flag table.
...
Test PC notes: Baelor (Illrigger/Hellspeaker Kalashtar) `PlayerCharacters/Test.md`; Gareth (SRD-2024 Fighter L1) `PlayerCharacters/Gareth the Bold.md`; the active-buff fixture `PlayerCharacters/R4-P2 Buff Fixture.md` (Illrigger L11 / Hellspeaker, boons Veil of Lies + Shadow Shroud), which depends on the custom race note `Compendium/MCDM/Races/Warden-Touched.md`. Both fixture files were authored by R4-P2b and live ONLY in this vault; no bundle ships them, so a vault reset loses them.

### The active-buff fixture, and the trap in it

`R4-P2 Buff Fixture.md` exists to light the FOUR active-buff render sites at once. They are mutually exclusive (no element is counted twice), and these counts were measured live on it:

| # | Site | Selector | Count |
|---|---|---|---|
| 1 | inline pool control | `#panel-pool-boons .pc-spell-prep-row .pc-pool-active` | 2 |
| 2 | active-effects rail | `#panel-pool-boons .pc-ae-tile` | 1 |
| 3 | boon-row Active toggle | `.pc-boon-row .pc-pool-active` | 2 |
| 4 | feature-row buff toggle | `#panel-actions .pc-action-buff-toggle` | 1 |

- **Site 4 is on the ACTIONS tab, NOT Passive.** R4-P2b amended the `warden-touched-veilstep` race trait with `action: bonus-action` precisely so it would surface there; without an action cost a race trait never reaches the feature-row buff toggle at all, and `#panel-passive .pc-action-buff-toggle` matches nothing. Anyone reusing this fixture who expects site 4 on Passive gets a false negative.
- **`active_buffs` lives INSIDE the ` ```pc ` fence, under `state:`, never in the frontmatter.** Put it in the frontmatter and the note still parses cleanly while two of the four sites go silently dark.
- **BOTH id vocabularies must be seeded**, because the two paths key on different strings: the pool and boon-row path on the full namespaced slug (`mcdm_optional-feature_veil-of-lies`), the feature-row path on the trait's own `id` (`warden-touched-veilstep`). Seed one and you light half the sites.
...
| `--assert-selector <sel>` | Fails unless at least one element matches `<sel>`. **The count is DOCUMENT-WIDE** (`document.querySelectorAll`), and `--within` does NOT scope it: `--within` scopes `--assert-text` only. Every tab panel is built on every sheet render and activating a tab merely toggles a CSS class, so an unscoped selector is satisfied by a match in a HIDDEN panel regardless of which tab is active. **Prefix it with the panel whenever the assertion is about a specific tab.** | `--assert-selector "#panel-passive .pc-weapon-mastery"` |
...
| `--click <sel>` · `--press-key Escape\|Enter\|Tab` · `--wait <ms>` · `--expect <sel>` · `--expect-absent <sel>` · `--shot <label>` | Ordered step verbs, collected by a walk over argv (so they may REPEAT and run in the order written) and run after the `--tab` click and before the screenshot. `--expect-absent` is the negative form `--assert-selector` lacks, so "Escape closes the modal" is expressible. **`--expect-absent` REQUIRES a paired `--expect` on the SAME selector string earlier in the list. That pairing is a requirement, not a stylistic preference in the example:** nothing validates the selector, so a typo matches nothing and the step passes unconditionally, and it passes just as happily against a modal that never opened. The `--expect` is the only thing proving the string can ever match. `--expect` and `--expect-absent` both count document-wide, exactly like `--assert-selector`. Keys go out as TRUSTED `Input.dispatchKeyEvent` events, because an untrusted `dispatchEvent` does not drive Obsidian's Keymap. Steps MUST be self-reverting: nothing left open, no note written. Declaring steps that never execute fails the run. | `--click ".pc-currency-clickable" --expect ".pc-coin-modal" --press-key Escape --expect-absent ".pc-coin-modal"` |
...
| Reading a bare `--assert-selector ".foo"` under `--tab passive` as "it is on the Passive tab" | `--assert-selector` is `document.querySelectorAll`, DOCUMENT-WIDE, and `--within` does not scope it. Every panel is built on every render and the tab click only toggles a CSS class, so the count is satisfied by a match in ANY panel, hidden or not. Measured live: a bare `--assert-selector ".pc-action-buff-toggle"` PASSED with the Passive tab active, on an element living in `#panel-actions`. Prefix the panel: `#panel-passive .foo` |
| A lone `--expect-absent ".modal"` recorded as "Escape closed it" | Nothing validates the selector, so a typo matches nothing and the step passes unconditionally, and so does a run where the modal never opened. Pair it with an `--expect` on the SAME string earlier in the step list, every time |
