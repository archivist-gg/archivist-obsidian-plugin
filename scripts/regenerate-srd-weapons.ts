// scripts/regenerate-srd-weapons.ts

import * as fs from "fs";
import * as path from "path";
import * as yaml from "js-yaml";
import { normalizeSrdWeapon } from "../src/shared/entities/srd-normalizer";

const SRD_JSON_DIR = "/Users/shinoobi/w/archivist/server/data/srd/weapons";
const COMPENDIUM_MD_DIR = "/Users/shinoobi/Documents/V/Compendium/SRD/Weapons";
...
function buildMarkdown(json: Record<string, unknown>): string {
  ...
  const normalized = normalizeSrdWeapon(json);
  ...
  const frontmatter = yaml.dump(
    { archivist: true, entity_type: "weapon", slug, name, compendium: "SRD" },
    { lineWidth: 999 },
  );
...
function main() {
  ...
  console.log(`Processing ${files.length} weapon files...`);
